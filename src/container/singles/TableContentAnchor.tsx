...
interface TableContentProps {
  ...
  className?: string
  btnClassName?: string
  renderTOC?: (headings: HeadingNode[]) => React.ReactNode
}
...
const TableContent: React.FC<TableContentProps> = ({
  ...
  const defaultRenderHeadings = (headings: HeadingNode[]) => {
    return (
      <>
            ...
            <li key={heading.id}>
              ...
              {heading?.children?.length ? (
                <ol className="mt-2 space-y-3 ps-4 text-neutral-500 dark:text-neutral-300">
                  {defaultRenderHeadings(heading.children)}
                </ol>
              ) : null}
            </li>
          )
      ...
      <nav>
        ...
        <div className="">
          <ol className="mt-4 space-y-3 text-sm">
            {renderTOC ? renderTOC(headingsWrapList) : defaultRenderHeadings(headingsWrapList)}
          </ol>
        </div>
      </nav>
    )
